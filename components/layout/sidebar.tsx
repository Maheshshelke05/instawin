    ...
    <aside className={cn("flex flex-col bg-sidebar text-sidebar-foreground", className)} {...props}>
      {/* Brand + Theme Toggle */}
            <div className="px-5 pt-6 pb-5 flex items-center gap-2.5">
              <div className="w-7 h-7 bg-accent-blue text-accent-blue-foreground rounded-md flex items-center justify-center shrink-0">
                <Zap className="w-3.5 h-3.5" strokeWidth={2.5} />
              </div>
              <span className="font-serif-display italic text-lg tracking-tight text-sidebar-foreground flex-1">InstaWin</span>
              <ThemeToggle />
            </div>
        ...
        {NAV.map(({ href, icon: Icon, label }) => {
          ...
          return (
            <Link
              ...
              className={cn(
                "flex items-center gap-3 px-3 py-2 rounded-md text-[13px] transition-all duration-200 relative focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sidebar-ring",
                active
            ...
            >
              {active && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-4 rounded-full bg-accent-yellow transition-all" />}
              <Icon className={cn("w-4 h-4 shrink-0 transition-colors", active ? "text-accent-yellow" : "")} strokeWidth={active ? 2.2 : 1.8} />
              <span>{label}</span>
            </Link>
          )
        })}
